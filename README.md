...
Glyph Wallet support uses `@glyph-oss/connect` v2.2.1 with the official relay. Keep `NEXT_PUBLIC_APP_ORIGIN` set to the public HTTPS origin because the hardened SDK rejects localhost, private-address origins, custom relay callbacks, mismatched nonces, and mismatched result types.
