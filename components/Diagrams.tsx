import { ArrowRight, Blocks, Braces, Code2, Database, Send, ShieldCheck, UserCheck, UserRound } from "lucide-react";
...
export function StackDiagram() { return <figure className="architecture"><header className="architecture-header"><p>System architecture</p><h2>Four layers. One maintained path.</h2><span>The product family is arranged by responsibility, from user control to network access.</span></header><div className="architecture-track">{layers.map((layer, index) => { const Icon = layer.icon; return <article className="architecture-stage" key={layer.name}><div className="architecture-stage-top"><span>0{index + 1}</span><Icon aria-hidden="true" /></div><div className="architecture-stage-copy"><h3>{layer.name}</h3><p>{layer.description}</p></div><ul>{layer.products.map((id) => { const product = productById[id]; return <li key={id}><span>{product.name.replace("Glyph ", "")}</span><small>{product.status}</small></li>; })}{layer.name === "Infrastructure" && <li className="architecture-boundary"><span>Qubic integrations</span><small>Network boundary</small></li>}</ul>{index < layers.length - 1 && <i className="architecture-connector" aria-hidden="true"><ArrowRight /></i>}</article>; })}</div><figcaption>Availability and license status remain explicit on each product page.</figcaption></figure> }
